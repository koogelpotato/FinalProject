...
enum ImGuiHoveredFlags_
{
    ...
    ImGuiHoveredFlags_NoSharedDelay =
        1 << 13, // Disable shared delay system where moving from one item to
                 // the next keeps the previous timer for a short time (standard
                 // for tooltips with long delays)
};
...
struct ImGuiIO
{
    ...
    // - consider using e.g. Win32's IsDebuggerPresent() as an additional filter
    // (or see ImOsIsDebuggerPresent() in imgui_test_engine/imgui_te_utils.cpp
    // for a Unix compatible version).
...
struct ImGuiListClipper
{
    ...
    // alignment / padding of certain items it is possible that an extra item
