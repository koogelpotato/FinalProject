...
enum ImGuiInputFlags_
{
    ...
    ImGuiInputFlags_LockThisFrame =
        1 << 6, // Access to key data will require EXPLICIT owner ID
                ...
                // at end of frame. This is useful to make input-owner-aware
                // code steal keys from non-input-owner-aware code.
    ...
    ImGuiInputFlags_RouteGlobal =
        1 << 10, // Register route globally (medium priority: unless an active
                 // item registered the route, e.g. CTRL+A registered by
                 // InputText).
...
enum ImGuiNavMoveFlags_
{
    ...
    ImGuiNavMoveFlags_NoSelect =
        1 << 12, // Don't trigger selection by not setting g.NavJustMovedTo
    ImGuiNavMoveFlags_NoSetNavHighlight =
        1 << 13, // Do not alter the visible state of keyboard vs mouse nav
                 // highlight
};
...
struct ImGuiContext
{
    ...
    float HoverDelayTimer; // Currently used IsItemHovered(), generally inferred
                           // from g.HoveredIdTimer but kept uncleared until
                           // clear timer elapse.
    float HoverDelayClearTimer; // Currently used IsItemHovered(): grace time
...
struct ImGuiTableInstanceData
{
    ...
    float
        LastFirstRowHeight; // Height of first row from last frame (FIXME: this
...
IMGUI_API void    SetItemKeyOwner(
       ...
       ImGuiInputFlags flags =
           0); // Set key owner to last item if it is hovered or active. Equivalent
               // to 'if (IsItemHovered() || IsItemActive()) { SetKeyOwner(key,
               // GetItemID());'.
